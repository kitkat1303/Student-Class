...
public class YoungHuman {
	...
	public YoungHuman(YoungHuman toCopy) {
		if (toCopy != null) {
			setName(this.firstName = toCopy.firstName, this.lastName = toCopy.lastName);
			this.birthDate = new Date(toCopy.getBirthDate());
			this.lastCheckUpDate = new Date(toCopy.getLastCheckUpDate()); 
	 ...
	 */
	private boolean isAfterBirthday(Date other) {
		if (other == null) {
			return false;
	 ...
	 */
	private boolean isBeforeCurrent(Date other) {
		if (other == null) {
			return false;
